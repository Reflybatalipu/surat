...
// ========================================================
// FUNGSI BERSAMA: render baris tabel + card + modal
// (dipakai oleh action=search dan action=get_folder_isi
// agar tidak duplikasi kode)
// ========================================================
function bangun_html_arsip($data_arsip, $lampiran_arsip, $role_sekarang, $no_awal)
{
    $no = $no_awal;
    $html_table = "";
    $html_card = "";
    ...
    foreach ($data_arsip as $data) {
        ...
        // 1. Render Baris Tabel (mode List)
        ...
        // 2. Render Card Layout (mode Card)
        $html_card .= '<div class="col-12 col-sm-6 col-lg-4">';
        $html_card .= '<div class="card border-0 shadow-sm rounded-4 mb-3 hover-arsip h-100" style="cursor: pointer;" data-bs-toggle="modal" data-bs-target="#modalDetail' . $jenis_id . $data['id'] . '">';
        ...
        if ($role_sekarang == 'Admin_TU') {
            ...
            $html_modal .= '<form action="earsip.php" method="POST">';
    ...
    return ['table' => $html_table, 'card' => $html_card, 'modal' => $html_modal];
}

// ========================================================
// FUNGSI BERSAMA: ambil data lampiran untuk kumpulan id surat masuk
// ========================================================
function ambil_lampiran($koneksi, $surat_masuk_ids)
{
    $lampiran_arsip = [];
    if (!empty($surat_masuk_ids)) {
        $ids_str = implode(',', array_map('intval', $surat_masuk_ids));
...
// ========================================================
// ACTION: get_folders
// Hitung jumlah surat per lokasi_fisik (folder), dipakai untuk
// merender grid folder di earsip.php
// ========================================================
if ($action === 'get_folders') {
    ...
    if ($q_folder) {
        while ($f = mysqli_fetch_assoc($q_folder)) {
            $data_folder[] = ['lokasi' => $f['lokasi'], 'jumlah' => (int) $f['jumlah']];
        }
    }

    header('Content-Type: application/json');
    echo json_encode(['folders' => $data_folder]);
    exit;
}

// ========================================================
// ACTION: get_folder_isi
// Ambil semua surat yang berada di 1 folder (lokasi_fisik) tertentu,
// lalu render table/card/modal HTML-nya (on-demand saat folder diklik)
// ========================================================
if ($action === 'get_folder_isi') {
    $lokasi_param = isset($_GET['lokasi_fisik']) ? $_GET['lokasi_fisik'] : '';

    if ($lokasi_param === 'Belum Dikelompokkan') {
        $cond_lokasi_masuk = "(lokasi_fisik IS NULL OR lokasi_fisik = '')";
        $cond_lokasi_keluar = "(lokasi_fisik IS NULL OR lokasi_fisik = '')";
    } else {
        $lokasi_esc = mysqli_real_escape_string($koneksi, $lokasi_param);
        $cond_lokasi_masuk = "lokasi_fisik = '$lokasi_esc'";
        $cond_lokasi_keluar = "lokasi_fisik = '$lokasi_esc'";
    }
    ...
    $sql_masuk = "SELECT id, nomor_surat, perihal, tanggal_surat AS tanggal, klasifikasi, lokasi_fisik, file_path, 'Surat Masuk' AS jenis, created_at 
                  FROM surat_masuk WHERE status_workflow IN ('Selesai', 'Diarsipkan') AND $cond_lokasi_masuk";
    $sql_keluar = "SELECT id, nomor_surat, perihal, tanggal_keluar AS tanggal, sifat_surat, lokasi_fisik, file_path, 'Surat Keluar' AS jenis, created_at 
                   FROM surat_keluar WHERE status_workflow = 'Terkirim' AND $cond_lokasi_keluar";
    ...
    $lampiran_arsip = ambil_lampiran($koneksi, $surat_masuk_ids);
    $hasil = bangun_html_arsip($data_arsip, $lampiran_arsip, $role_sekarang, 1);
...
// Perilaku lama — pencarian keyword & infinite scroll flat list.
// TIDAK DIUBAH agar kompatibel dengan behavior yang sudah berjalan.
// ========================================================
$limit = 30;
...
$lampiran_arsip = ambil_lampiran($koneksi, $surat_masuk_ids);
$hasil = bangun_html_arsip($data_arsip, $lampiran_arsip, $role_sekarang, $offset + 1);
