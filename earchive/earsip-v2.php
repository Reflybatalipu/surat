...
// --- Penggabungan Tabel (UNION) dengan LIMIT AWAL 30 DATA ---
$sql_final = "";
if ($filter_jenis == 'Surat Masuk') {
    $sql_final = $sql_masuk . " ORDER BY created_at DESC LIMIT 30 OFFSET 0";
} elseif ($filter_jenis == 'Surat Keluar') {
    $sql_final = $sql_keluar . " ORDER BY created_at DESC LIMIT 30 OFFSET 0";
} else {
    $sql_final = "($sql_masuk) UNION ALL ($sql_keluar) ORDER BY created_at DESC LIMIT 30 OFFSET 0";
}
...
<div class="card border-0 shadow-sm rounded-3 mb-4 bg-light">
    <div class="card-body">
        <form action="" method="GET" class="row g-2 align-items-center">
            ...
            <div class="col-md-7">
                <input type="text" class="form-control" name="cari" id="filter_cari" value="<?= htmlspecialchars($keyword); ?>" placeholder="Cari nomor, perihal, atau isi teks dalam surat (OCR)...">
            </div>
        ...
        <?php else: ?>
            ...
            <div class="table-responsive d-none d-md-block">
                <table class="table table-hover align-middle">
                    ...
                    <tbody id="container-tabel-arsip">
                        <?php 
                        $no = 1;
            ...
            <div class="d-block d-md-none p-3 bg-light" id="container-card-arsip">
            ...
            <div id="loading-infinite" class="text-center py-3 text-muted d-none">
                <div class="spinner-border spinner-border-sm text-primary me-2" role="status"></div>
                Memuat arsip lainnya...
            </div>
...
<script>
// 🛡️ PENGERTIAN BLOK PENGAMAN GLOBAL JAVASCRIPT
// Membungkus inisialisasi pdfjs worker ke dalam try-catch agar crash vendor tidak membunuh script infinite scroll
try {
    if (typeof pdfjsLib !== 'undefined') {
        // Mengarahkan ke CDN eksternal resmi jika file lokal hosting .mjs kamu diblokir oleh server hosting
        pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.worker.min.js';
    }
} catch (e) {
    console.warn("PDF.js Worker gagal diinisialisasi secara normal, dialihkan ke fallback safety mechanism.", e);
}
...
const currentJenis = "<?= $filter_jenis; ?>";
const currentCari = "<?= $keyword; ?>";

function loadMoreData() {
    if (isLoading || isDataExhausted) return;
    
    isLoading = true;
    const loader = document.getElementById('loading-infinite');
    if(loader) loader.classList.remove('d-none');

    fetch(`ambil_arsip_ajax.php?offset=${offset}&jenis=${encodeURIComponent(currentJenis)}&cari=${encodeURIComponent(currentCari)}`)
        .then(response => {
            if (!response.ok) {
                throw new Error('Gagal memuat halaman AJAX');
            }
        ...
        .then(data => {
            if (!data.table || data.table.trim() === "") {
                ...
                if(loader) {
                    ...
                    loader.innerHTML = "<span class='text-muted small fw-bold'>Semua data arsip telah selesai ditampilkan.</span>";
                }
                return;
            }

            const tblContainer = document.getElementById('container-tabel-arsip');
            const cardContainer = document.getElementById('container-card-arsip');
            const modalContainer = document.getElementById('container-modal-arsip');

            if(tblContainer && data.table) tblContainer.insertAdjacentHTML('beforeend', data.table);
            if(cardContainer && data.card) cardContainer.insertAdjacentHTML('beforeend', data.card);
            if(modalContainer && data.modal) modalContainer.insertAdjacentHTML('beforeend', data.modal);
        ...
        .catch(error => {
            console.error("Terjadi interupsi data infinite scroll:", error);
            isLoading = false;
            if(loader) loader.classList.add('d-none');
        });
}

// Detektor Sensor Scroll Utama (Desktop & Mobile) menggunakan sensitivitas 300px sebelum footer halaman berakhir
