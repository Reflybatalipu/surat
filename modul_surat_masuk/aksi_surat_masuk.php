...
function save_base64_pdf($base64, $upload_dir, $prefix) {
...
// ====================================================================
//  DETEKSI AKSI — tetap kompatibel walau tombol submit di-disable JS
// ====================================================================
$is_post = ($_SERVER['REQUEST_METHOD'] === 'POST');
$is_tambah = isset($_POST['tambah']) || ($is_post && !isset($_POST['id_surat']) && isset($_POST['nomor_surat'], $_POST['pengirim'], $_POST['perihal']));
$is_hapus = isset($_POST['hapus']) || ($is_post && isset($_POST['id']) && !isset($_POST['id_surat']) && !isset($_POST['nomor_surat']));
$is_edit = isset($_POST['edit_surat']) || ($is_post && isset($_POST['id_surat'], $_POST['nomor_surat'], $_POST['pengirim'], $_POST['perihal']));
...
if ($is_tambah) {
    ...
    if ($mode_utama === 'kamera' && !empty($_POST['surat_utama_base64'])) {
        $save = save_base64_pdf($_POST['surat_utama_base64'], $upload_dir, 'SM');
    ...
    if ($mode_lampiran === 'kamera' && !empty($_POST['lampiran_base64'])) {
        $save_lamp = save_base64_pdf($_POST['lampiran_base64'], $upload_dir, 'Lampiran');
        if ($save_lamp['ok']) {
...
if ($is_edit) {
    ...
    $unit_tujuan_id = (int)($_POST['unit_tujuan_id'] ?? 0);
    $perihal        = safe_input($koneksi, 'perihal');
    $file_lama      = basename($_POST['file_lama'] ?? '');
